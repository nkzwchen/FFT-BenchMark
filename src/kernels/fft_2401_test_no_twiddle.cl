...
#define BACKWARD_SCALE 1.0000000000000000000000000000000000e+00
#define C7Q1 -1.16666666666666651863693004997913f
#define C7Q2  0.79015646852540022404554065360571f
#define C7Q3  0.05585426728964774240049351305970f
#define C7Q4  0.73430220123575240531721419756650f
#define C7Q5  0.44095855184409837868031445395900f
#define C7Q6  0.34087293062393136944265847887436f
#define C7Q7 -0.53396936033772524066165487965918f
#define C7Q8  0.87484229096165666561546458979137f

#if DIRECTION == 1
#define BasicDFT7(R0, R1, R2, R3, R4, R5, R6)              \
{                                                           \
    float2 p0, p1, p2, p3, p4, p5, p6, p7, p8, p9;            \
    float2 q0, q1, q2, q3, q4, q5, q6, q7, q8;               \
                                                            \
    p0 = R1 + R6;                                         \
    p1 = R1 - R6;                                         \
    p2 = R2 + R5;                                         \
...
#if DIRECTION == 1
#define DirectionComplexMultiply(R, tw, tmp){\
    tmp.even = tw.even * (R).even;\
    tmp.even -= tw.odd * (R).odd;\
    tmp.odd = tw.even * (R).odd;\
    tmp.odd += tw.odd * (R).even;\
    R = tmp;\
}
#else
#define DirectionComplexMultiply(R, tw, tmp){\
...
#define ComplexMultiply(tw1, tw2, tmp){\
    tmp.even = tw1.even * (tw2).even;\
    tmp.even -= tw1.odd * (tw2).odd;\
    tmp.odd = tw1.even * (tw2).odd;\
    tmp.odd += tw1.odd * (tw2).even;\
    tw1 = tmp;\
}
...
void micro_pass_0(__private uint rw, __private uint lid, __global float2* src, __local float2* dest)
{
    ...
    {
        {
            {BasicDFT7(R0, R1, R2, R3, R4, R5, R6)}
        }

        {
            {BasicDFT7(R7, R8, R9, R10, R11, R12, R13)}
        }

        {
            {BasicDFT7(R14, R15, R16, R17, R18, R19, R20)}
        }

        {
            {BasicDFT7(R21, R22, R23, R24, R25, R26, R27)}
        }

        {
            {BasicDFT7(R28, R29, R30, R31, R32, R33, R34)}
        }

        {
            {BasicDFT7(R35, R36, R37, R38, R39, R40, R41)}
        }

        {
            {BasicDFT7(R42, R43, R44, R45, R46, R47, R48)}
        }
...
void fft_2401_test_no_twiddle(__global float2* input, __global float2* output)
{
