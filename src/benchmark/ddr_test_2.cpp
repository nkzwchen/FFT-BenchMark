...
#define BLOCK_NUM (FFT_LENGTH) / (THREAD_PER_BLOCK * ELEMENT_PER_THREAD)
#define STRIDE THREAD_PER_BLOCK
...
void ddr_test_2(cl_device_id& device,  cl_command_queue& que, cl_context& context){
    ...
    // 读取并编译Kernel
    char* source_code = ReadKernelSource("../src/kernels/ddr_test_2.cl", macro_definitions_str);
    ...
    size_t row_thread = BLOCK_NUM * THREAD_PER_BLOCK;

    size_t global_work_size[3] = {row_thread , batch, 1};
