...
void l1_cacheline_test(cl_device_id& device,  cl_command_queue& que, cl_context& context){
    ...
    // 读取并编译Kernel
    char* source_code = ReadKernelSource("../src/kernels/l1_cacheline_test.cl", macro_definitions_str);
    ...
    // 创建Kernel
    cl_int ret;
    cl_kernel kernel = clCreateKernel(program, "l1_cacheline_test", &ret);
    ...
    size_t row_thread = BLOCK_ROW_LENGTH * BLOCK_NUM;
