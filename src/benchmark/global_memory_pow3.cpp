...
void test_global_memory_pow3(cl_device_id& device,  cl_command_queue& que, cl_context& context){
    ...
    // 读取并编译Kernel
    char* source_code = ReadKernelSource("../src/kernels/global_memory_pow3.cl", macro_definitions_str);
    ...
    // 创建Kernel
    cl_int ret;
    cl_kernel kernel = clCreateKernel(program, "global_memory_pow3", &ret);
