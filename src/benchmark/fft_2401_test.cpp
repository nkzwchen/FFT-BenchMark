...
void fft_2401_test(cl_device_id& device,  cl_command_queue& que, cl_context& context){
    size_t fft_length = FFT_LENGTH;
    size_t batch = 256;    

    // 读取并编译Kernel
    char* source_code = ReadKernelSource("../src/kernels/fft_2401_test.cl");
    ...
    // 创建Kernel
    cl_int ret;
    cl_kernel kernel = clCreateKernel(program, "fft_2401_test", &ret);
