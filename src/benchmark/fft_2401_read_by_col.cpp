...
void fft_2401_read_by_col(cl_device_id& device,  cl_command_queue& que, cl_context& context){
    ...
    // 读取并编译Kernel
    char* source_code = ReadKernelSource("../src/kernels/fft_2401_read_by_col.cl");
    ...
    // 创建Kernel
    cl_int ret;
    cl_kernel kernel = clCreateKernel(program, "fft_729_test", &ret);
